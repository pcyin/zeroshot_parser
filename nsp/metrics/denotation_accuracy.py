    ...
    def get_metric(self, reset: bool = False) -> Dict[str, float]:
        ...
        if reset and all_lfs:
            ...
            if self._domain == "geo":
                ...
                subdomain = "geo880"
            elif self._domain == 'scholar_old':
                evaluator_name = "evaluator/scholar.bak"
                subdomain = "external"
            else:
                # evaluator_name = "evaluator/scholar"
                evaluator_name = "new_evaluator/scholar_parallel_debug"
                subdomain = "external"
            start_time = time.time()
            f_err = open(tf.name + '.sempre.out', 'w')
            print(f'Saving sempre output to {tf.name + ".sempre.out"}')
            try:
                msg = subprocess.check_output([evaluator_name, subdomain, tf.name], stderr=f_err)
            ...
            # msg_lines = msg.decode("utf-8").split("\n")
            # print('Sempre output: ')
            # for line in msg_lines:
            #     print(line)
            f_err.write(msg.decode("utf-8"))
            f_err.close()
            ...
            assert len(denotations) == len(tf_lines)
        ...
        return {"den_acc": accuracy, 'ind_to_match': ind_to_match}


def lexicalize_entity(var, token):
    """Lexicalize an abstract entity"""
    curr_year = 2016
    if token in {"year0", "year1"} and (token in var or "misc0" in var):
        if token in var:
