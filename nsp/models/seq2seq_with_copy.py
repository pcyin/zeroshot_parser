...
from nsp.models.stacked_lstm_cell import StackedLSTMCell
...
class Seq2SeqModelWithCopy(Model):
    """
    ...
    def __init__(
        ...
        denotation_metric: DenotationAccuracyProxy = None,
        domain: str = None,
        validation_metric: str = 'seq_acc',
        tgt_stopword_loss_weight: Optional[float] = None,
        ...
        assert validation_metric in ['seq_acc', 'ppl', 'denotation_acc']
        self._validation_metric = validation_metric
        self._perplexity_metric = Perplexity()

        self._tgt_stopword_loss_weight = tgt_stopword_loss_weight
    ...
    def forward(
        ...
        if not self.training and self._prediction:
            state = self._init_decoder_state(state)
            if getattr(self, '_validation') and self._validation_metric == 'ppl':
                assert target_tokens is not None
                loss = output_dict['weighted_loss'].detach()
                batch_size = target_tokens_mask.size(0)
                total_num_tgt_tokens = target_tokens_mask[:, 1:].sum()
                batch_per_token_loss = loss * batch_size / total_num_tgt_tokens

                self._perplexity_metric(batch_per_token_loss)
            else:
                ...
                if target_tokens:
                    ...
                    if self._token_based_metric:
                        self._token_based_metric(hyp_tokens, [x["target_tokens"] for x in metadata])
                    if self._validation_metric == 'denotation_acc':
                        assert self._denotation_metric is not None
                        ...
                        self._denotation_metric(  # type: ignore
                            [
                                [hyp] for hyp in hyp_tokens
                            ],
                            [
                                x["target_tokens"] for x in metadata
                            ],
                            [
                                x["variables"] for x in metadata
                            ],
                            [
                                x["index"] for x in metadata
                            ]
                        )

        del output_dict['weighted_loss']
    ...
    def _forward_loss(
        ...
        if self._tgt_stopword_loss_weight is not None:
            stop_word_idx = [
                self.vocab.get_token_index('(', namespace='target_tokens'),
                self.vocab.get_token_index(')', namespace='target_tokens')
            ]

            shifted_target_tokens_id = target_tokens_id[:, 1:]
            target_tokens_is_stop_word = ((shifted_target_tokens_id == stop_word_idx[0]) | (shifted_target_tokens_id == stop_word_idx[1])).float()
            weighted_step_log_likelihoods = (
                (
                    (self._tgt_stopword_loss_weight * target_tokens_is_stop_word) * step_log_likelihoods +
                    (1.0 - target_tokens_is_stop_word) * step_log_likelihoods
                ) * target_tokens_mask[:, 1:]
            )
        else:
            weighted_step_log_likelihoods = step_log_likelihoods

        weighted_loss = -weighted_step_log_likelihoods.detach().sum() / batch_size
        ...
        output_dict["example_log_probs"] = tgt_log_likelihood.detach().cpu()
        output_dict["weighted_loss"] = weighted_loss
    ...
    def _forward_decode(
        ...
        for time_step in range(num_decoding_steps):
            ...
            att_t, state, att_weight_t = self._decoder_step(x_t, state, return_attention=True)
    ...
    def _decoder_step(
        ...
        decoder_output_t, (h_t, cell_t) = self._decoder_cell(
            x_t, (state_tm1['decoder_h'], state_tm1['decoder_cell'])
        )
    ...
    def _get_forward_log_likelihoods(
        ...
        scores_mask = generation_scores_mask
    ...
    def _beam_search_step(
        ...
        # print(state['decoder_hidden'].shape)

        # Update the decoder state by taking a step through the RNN.
        att_vec, state, attention_weights = self._decoder_step(x_t, state, return_attention=True)
        ...
        # print(2, state['decoder_hidden'].shape)
    ...
    def get_metrics(self, reset: bool = False) -> Dict[str, float]:
        ...
        if not self.training:
            ...
            if self._validation_metric == 'ppl':
                all_metrics['ppl'] = self._perplexity_metric.get_metric(reset=reset)

            if self._validation_metric == 'denotation_acc':
                all_metrics['denotation_acc'] = self._denotation_metric.get_metric(reset=reset)['den_acc']
